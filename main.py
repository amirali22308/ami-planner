...
def main():
    ...
    pg.display.set_icon(pg.image.load(path + "images/logo.png"))
    all_text = []
    all_images = []
    all_lines = []
    ...
    target_func = [None]
    start_of_line = [None]
    make_line_i = pg.image.load(path + "images/add_l.png")
    ...
    delete_line_i = pg.image.load(path + "images/delete_l.png")
    def delete_line(pos):
        target = None
    ...
    make_image_i = pg.image.load(path + "images/add_i.png")
    ...
    make_text_i = pg.image.load(path + "images/add_t.png")
    def create_text(pos):
        all_text.append(text_item(get_input("insert text"),pos,font))
    target_item_to_move = [None]
    move_item_i = pg.image.load(path + "images/edit_pos.png")
    ...
    change_scale__i = pg.image.load(path + "images/edit_scale.png")
    def change_scale(pos):
        for item in all_text:
            rect = item.render.get_rect()
            if item.pos[0] <= pos[0] and pos[0] <= (item.pos[0]+rect.width):
                if item.pos[1] <= pos[1] and pos[1] <= (item.pos[1]+rect.height):
                    item.new_scale(float(get_input()))
                    return
        for item in all_images:
            rect = item.render.get_rect()
            if item.pos[0] <= pos[0] and pos[0] <= (item.pos[0]+rect.width):
                if item.pos[1] <= pos[1] and pos[1] <= (item.pos[1]+rect.height):
                    ...
                    return
    change_text_i = pg.image.load(path + "images/edit_t.png")
    def change_text(pos):
        for item in all_text:
            rect = item.render.get_rect()
            if item.pos[0] <= pos[0] and pos[0] <= (item.pos[0]+rect.width):
                if item.pos[1] <= pos[1] and pos[1] <= (item.pos[1]+rect.height):
                    ...
                    break 
    delete_item_i = pg.image.load(path + "images/delete_item.png")
    def delete_item(pos):
        remove_t = None
        remove_i = None
        for item in all_text:
    ...
    save_i = pg.image.load(path + "images/save_i.png")
    def save_b():
        tk.Tk().withdraw()
        save(all_text,all_images,all_lines,filedialog.askdirectory())
    load_i = pg.image.load(path + "images/load_i.png")
    def load_b():
        tk.Tk().withdraw()
        result = load(filedialog.askopenfilename(
        initialdir = "/",title = "Select an Saved plan"
        ,filetypes = (("ap files", "*.ap*"),)),font)
        if result != None:
            all_text.clear()
    ...
    while loop:
        ...
        win.blit(save_i,(565,item_h))
        save_r = save_i.get_rect()


        win.blit(load_i,(635,item_h))
        load_r = load_i.get_rect()
            ...
            if e.type == pg.MOUSEBUTTONDOWN:
                ...
                if e.button == 1:
                    if last_mouse_pos[1] <= hot_bar_h * screensize[1]:
                        ...
                        elif 565 <= last_mouse_pos[0] and last_mouse_pos[0] <= 565+save_r.width:
                            if item_h <= last_mouse_pos[y] and last_mouse_pos[y] <= item_h+save_r.height:
                                save_b()
                                target_func = None
                        elif 635 <= last_mouse_pos[0] and last_mouse_pos[0] <= 635+load_r.width:
                            if item_h <= last_mouse_pos[y] and last_mouse_pos[y] <= item_h+load_r.height:
                                load_b()
                                target_func = None
                        else:
                            target_func = None
